#!/usr/bin/env python3
import json
import re

# 读取书源数据
def load_book_sources():
    file_path = "samples/booksources/raw_online_dump/all_sources.json"
    with open(file_path, 'r', encoding='utf-8') as f:
        return json.load(f)

# 检查是否包含禁止内容
def contains_forbidden_content(text):
    if not text:
        return False
    
    # 检查是否包含 JS
    if '@js' in text or '<js>' in text or 'java.ajax' in text:
        return True
    
    # 检查是否包含 JSONPath
    if '$.data' in text or '$.title' in text or '$.chapterid' in text:
        return True
    
    # 检查是否包含 api/ajax
    if re.search(r'api|ajax', text, re.IGNORECASE):
        return True
    
    # 检查是否包含 cookie/token
    if re.search(r'cookie|token', text, re.IGNORECASE):
        return True
    
    return False

# 检查选择器是否符合 V3 能力要求
def is_valid_selector(selector):
    if not selector:
        return True
    
    if contains_forbidden_content(selector):
        return False
    
    # 检查是否包含 XPath
    if selector.startswith('//'):
        return False
    
    # 检查是否包含多层 descendant
    if selector.count(' ') > 1:
        return False
    
    return True

# 检查书源是否有实际规则
def has_actual_rules(source):
    rule_content = source.get('ruleContent', {})
    if isinstance(rule_content, dict) and rule_content.get('content'):
        return True
    
    rule_toc = source.get('ruleToc', {})
    if isinstance(rule_toc, dict) and (rule_toc.get('chapterList') or rule_toc.get('chapterUrl')):
        return True
    
    rule_book_info = source.get('ruleBookInfo', {})
    if isinstance(rule_book_info, dict) and rule_book_info.get('tocUrl'):
        return True
    
    return False

# 检查书源是否符合 V3 能力要求
def is_v3_compatible(source):
    # 检查是否有实际规则
    if not has_actual_rules(source):
        return False
    
    # 检查 ruleContent
    rule_content = source.get('ruleContent', {})
    if isinstance(rule_content, str):
        rule_content = {}
    content = rule_content.get('content', '')
    next_content_url = rule_content.get('nextContentUrl', '')
    
    if content and not is_valid_selector(content):
        return False
    if next_content_url and not is_valid_selector(next_content_url):
        return False
    
    # 检查 ruleToc
    rule_toc = source.get('ruleToc', {})
    if isinstance(rule_toc, str):
        rule_toc = {}
    chapter_list = rule_toc.get('chapterList', '')
    chapter_name = rule_toc.get('chapterName', '')
    chapter_url = rule_toc.get('chapterUrl', '')
    
    if chapter_list and not is_valid_selector(chapter_list):
        return False
    if chapter_name and not is_valid_selector(chapter_name):
        return False
    if chapter_url and not is_valid_selector(chapter_url):
        return False
    
    # 检查 ruleBookInfo
    rule_book_info = source.get('ruleBookInfo', {})
    if isinstance(rule_book_info, (list, str)):
        rule_book_info = {}
    toc_url = rule_book_info.get('tocUrl', '')
    
    if toc_url and not is_valid_selector(toc_url):
        return False
    
    # 检查 searchUrl
    search_url = source.get('searchUrl', '')
    if search_url and contains_forbidden_content(search_url):
        return False
    
    # 检查 ruleSearch
    rule_search = source.get('ruleSearch', {})
    if isinstance(rule_search, str):
        rule_search = {}
    for key, value in rule_search.items():
        if value and contains_forbidden_content(str(value)):
            return False
    
    return True

# 审计书源
def audit_source(source):
    source_name = source.get('bookSourceName', 'Unknown')
    bookSourceUrl = source.get('bookSourceUrl', 'Unknown')
    searchUrl = source.get('searchUrl', '')
    ruleSearch = source.get('ruleSearch', {})
    ruleBookInfo = source.get('ruleBookInfo', {})
    ruleToc = source.get('ruleToc', {})
    ruleContent = source.get('ruleContent', {})
    
    # 输出原始规则
    print(f"\n=== {source_name} ===")
    print(f"source_name: {source_name}")
    print(f"bookSourceUrl: {bookSourceUrl}")
    print(f"searchUrl: {searchUrl}")
    print(f"ruleSearch: {json.dumps(ruleSearch, ensure_ascii=False, indent=2)}")
    print(f"ruleBookInfo: {json.dumps(ruleBookInfo, ensure_ascii=False, indent=2)}")
    print(f"ruleToc: {json.dumps(ruleToc, ensure_ascii=False, indent=2)}")
    print(f"ruleContent: {json.dumps(ruleContent, ensure_ascii=False, indent=2)}")
    
    # 审计规则
    issues = []
    
    # 1. ruleContent.content
    content = ruleContent.get('content', '')
    if content and not is_valid_selector(content):
        issues.append("ruleContent.content 不符合 V3 能力要求")
    
    # 2. ruleToc.chapterList
    chapter_list = ruleToc.get('chapterList', '')
    if chapter_list and not is_valid_selector(chapter_list):
        issues.append("ruleToc.chapterList 不符合 V3 能力要求")
    
    # 3. ruleToc.chapterName
    chapter_name = ruleToc.get('chapterName', '')
    if chapter_name and not is_valid_selector(chapter_name):
        issues.append("ruleToc.chapterName 不符合 V3 能力要求")
    
    # 4. ruleToc.chapterUrl
    chapter_url = ruleToc.get('chapterUrl', '')
    if chapter_url and not is_valid_selector(chapter_url):
        issues.append("ruleToc.chapterUrl 不符合 V3 能力要求")
    
    # 5. ruleBookInfo.tocUrl
    if isinstance(ruleBookInfo, dict):
        toc_url = ruleBookInfo.get('tocUrl', '')
        if toc_url and not is_valid_selector(toc_url):
            issues.append("ruleBookInfo.tocUrl 不符合 V3 能力要求")
    
    # 6. 检查是否包含禁止内容
    all_rules = [
        searchUrl,
        json.dumps(ruleSearch, ensure_ascii=False),
        json.dumps(ruleBookInfo, ensure_ascii=False),
        json.dumps(ruleToc, ensure_ascii=False),
        json.dumps(ruleContent, ensure_ascii=False)
    ]
    
    for rule in all_rules:
        if contains_forbidden_content(rule):
            issues.append("包含禁止内容")
            break
    
    # 输出审计结果
    if issues:
        print(f"\nREJECTED_WITH_REASON: {'; '.join(issues)}")
        return False
    else:
        print("\nVERIFIED_V3_CANDIDATE")
        return True

# 主函数
def main():
    book_sources = load_book_sources()
    
    # 筛选 V3 兼容的书源
    v3_compatible = []
    for source in book_sources:
        if is_v3_compatible(source):
            v3_compatible.append(source)
    
    print(f"找到 V3 兼容书源: {len(v3_compatible)}")
    
    # 审计前 5 个兼容书源
    top_5 = v3_compatible[:5]
    verified_count = 0
    
    for source in top_5:
        if audit_source(source):
            verified_count += 1
    
    print(f"\n=== 审计结果 ===")
    print(f"验证通过: {verified_count}/5")

if __name__ == "__main__":
    main()