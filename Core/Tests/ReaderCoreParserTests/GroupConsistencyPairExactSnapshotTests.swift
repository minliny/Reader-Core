...
final class GroupConsistencyPairExactSnapshotTests: XCTestCase {
    private struct IssueSnapshot: Decodable, Encodable, Equatable {
        let code: String
        let keys: [String]
        let values: [String]
        let evidence: GroupConsistencyPairExactResult.Evidence
    }
    ...
    func testDuplicateHitSnapshot() throws {
        let result = try runSample("gc_pair_duplicate_hit")
        ...
        let expected = try loadSnapshot("gc_pair_duplicate_hit.snapshot.json", as: ResultSnapshot.self)
        XCTAssertEqual(snapshot, expected)
    }

    func testUniquePassSnapshot() throws {
        let result = try runSample("gc_pair_unique_pass")
        let snapshot = ResultSnapshot(
            pass: result.pass,
            checkedKeys: result.checkedKeys,
            checkedCount: result.checkedCount,
        ...
        let expected = try loadSnapshot("gc_pair_unique_pass.snapshot.json", as: ResultSnapshot.self)
        XCTAssertEqual(snapshot, expected)
    }

    func testStableOrderingSnapshot() throws {
        let result = try runSample("gc_pair_stable_ordering")
        ...
        let expected = try loadSnapshot("gc_pair_stable_ordering.snapshot.json", as: OrderingSnapshot.self)
        XCTAssertEqual(snapshot, expected)
    }

    func testMultipleGroupsSnapshot() throws {
        let result = try runSample("gc_pair_multiple_duplicate_tuple_groups")
        ...
        let expected = try loadSnapshot("gc_pair_multiple_duplicate_tuple_groups.snapshot.json", as: MultiGroupSnapshot.self)
        XCTAssertEqual(snapshot, expected)
    }

    func testMissingOneKeyRejectSnapshot() throws {
        try assertRejectSnapshot(
            sample: "gc_pair_missing_one_key_reject",
            snapshotFile: "gc_pair_missing_one_key_reject.snapshot.json"
        )
    }

    func testInvalidOneKeyRejectSnapshot() throws {
        try assertRejectSnapshot(
            sample: "gc_pair_invalid_one_key_reject",
            snapshotFile: "gc_pair_invalid_one_key_reject.snapshot.json"
        )
    }

    func testInvalidThreeKeysRejectSnapshot() throws {
        try assertRejectSnapshot(
            sample: "gc_pair_invalid_three_keys_reject",
            snapshotFile: "gc_pair_invalid_three_keys_reject.snapshot.json"
        )
    }

    func testDuplicateKeyNamesRejectSnapshot() throws {
        try assertRejectSnapshot(
            sample: "gc_pair_duplicate_key_names_reject",
            snapshotFile: "gc_pair_duplicate_key_names_reject.snapshot.json"
        )
    }

    private func runSample(_ sample: String) throws -> GroupConsistencyPairExactResult {
        let fixture = try loadFixture(sample)
        return try GroupConsistencyPairExactConstraint().evaluate(
            rule: fixture.rule,
    ...
    private func assertRejectSnapshot(sample: String, snapshotFile: String) throws {
        ...
        XCTAssertThrowsError(
            ...
            let actual = RejectSnapshot(
                ...
                error: .init(code: pairError.snapshotCode, messagePrefix: pairError.snapshotMessagePrefix)
            )
    ...
    private func snapshotURL(_ file: String) -> URL {
        repoRoot()
            .appendingPathComponent("samples/expected/group_consistency_pair/snapshots")
